...
public class TailBehavior : MonoBehaviour {
	...
	void MGlobal_OnLevelEnd()
	{
		...
		//go.GetComponent<BurstManager>().MakeBurst(4, Color.black, this.transform.position, this.transform.localScale.x, global::BurstManager.SpriteTextures.Circle);
	...
	IEnumerator FadeSprite()
	{
		...
		var go = Instantiate(BurstManager);
		go.GetComponent<BurstManager>().MakeBurst(5, Color.black, this.transform.position, this.transform.localScale.x, global::BurstManager.SpriteTextures.Circle);
		Destroy(this.gameObject);
		yield return null;
	}
